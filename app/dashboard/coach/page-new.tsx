...
function CoachDashboardContent() {
  ...
  const loadCoachData = async () => {
    try {
      ...
      // 加载KPI数据
      const kpiResponse = await authenticatedFetch('http://localhost:8787/api/dashboard/coach/kpis');
      if (kpiResponse.ok) {
      ...
      // 加载我的学员
      const studentsResponse = await authenticatedFetch('http://localhost:8787/api/dashboard/coach/students');
